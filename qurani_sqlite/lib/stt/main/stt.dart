...
import 'package:vosk_stt/stt/mushaf_renderer.dart';
...
class _VoskSTTCorrectionPageState extends State<VoskSTTCorrectionPage>
  ...
  bool _isUIVisible = true; // ← TAMBAHKAN BARIS INI
  ...
  // ==================== SURAH GLYPH HELPER ====================
String _formatSurahIdForGlyph(int surahId) {
  // Format ID surat untuk font glyph
  // Jika <= 9: "surah00X"
  // Jika >= 10 && <= 99: "surah0XX"
  // Jika >= 100: "surahXXX"
  
  if (surahId <= 9) {
    return 'surah00$surahId';
  } else if (surahId <= 99) {
    return 'surah0$surahId';
  } else {
...
Widget _buildVerticalModeHeader() {
  if (widget.suratId == null || widget.suratId <= 0) return const SizedBox.shrink();
  ...
  return Container(
    ...
    child: Stack(
      ...
      children: [
        ...
        Text(
          _formatSurahIdForGlyph(widget.suratId), // Menggunakan ID yang sudah diformat
    ...
    return GestureDetector(
      onHorizontalDragEnd: (DragEndDetails details) {
        if (details.velocity.pixelsPerSecond.dx > 300) {
          if (_currentPage > 1) {
            _navigateToPage(_currentPage + 1); // Fixed: swipe right = prev page
          }
        } else if (details.velocity.pixelsPerSecond.dx < -300) {
          if (_currentPage < 604) {
            _navigateToPage(_currentPage - 1); // Fixed: swipe left = next page
          }
        }
      },
child: FutureBuilder<List<MushafPageLine>>(
  future: _sqliteService.getMushafPageLines(_currentPage),
  builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      // Return previous content or empty container while loading
      return Container(
        height: MushafTextRenderer.PAGE_HEIGHT,
    ...
    if (!snapshot.hasData || snapshot.data!.isEmpty) {
      return Container(
        height: MushafTextRenderer.PAGE_HEIGHT,
        child: const Center(child: Text('No content for this page')),
      );
    }

    final pageLines = snapshot.data!;

          // Fixed height container for consistent 15-line layout
          return Container(
            height: MushafTextRenderer.PAGE_HEIGHT,
            // decoration: BoxDecoration(
            ...
            child: Padding(
              padding: EdgeInsets.symmetric(
                horizontal: MushafTextRenderer.PAGE_PADDING,
              ),
              child: Column(
                children: [
                  // Page header
                  ...
                  // Render exactly 15 lines
                  Expanded(child: Column(children: _buildPageLines(pageLines))),
                ],
              ),
            ),
          );
  ...
  // Build page lines ensuring exactly 15 lines
  List<Widget> _buildPageLines(List<MushafPageLine> pageLines) {
    List<Widget> widgets = [];

    for (final line in pageLines) {
      widgets.add(_buildMushafLine(line));
    }

    // Fill remaining lines with empty space to maintain 15-line structure
    while (widgets.length < 15) {
      widgets.add(SizedBox(height: MushafTextRenderer.LINE_HEIGHT));
    }

    return widgets;
  }

  // Add this method to the _VoskSTTCorrectionPageState class
  // Insert it in the "UTILITY METHODS" section around line 3200

  // ==================== JUZ CALCULATION METHOD ====================

  int _calculateJuz(int surahId, int ayahNumber) {
    // Accurate Juz boundaries based on Quran structure
    const List<Map<String, dynamic>> juzBoundaries = [
      ...
      {'juz': 4, 'surah': 3, 'ayah': 93}, // Ali 'Imran
      ...
      {'juz': 9, 'surah': 7, 'ayah': 88}, // Al-A'raf
      ...
      {'juz': 17, 'surah': 21, 'ayah': 1}, // Al-Anbya
  ...
  // Build page header with juz and page info
  Widget _buildPageHeader() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(
          'Juz ${(_calculateJuz(_currentPageAyats.isNotEmpty ? _currentPageAyats.first.surah_id : 1, _currentPageAyats.isNotEmpty ? _currentPageAyats.first.ayah : 1))}',
          style: TextStyle(
            fontSize: 14,
  ...
  // Build individual line based on type
  Widget _buildMushafLine(MushafPageLine line) {
    switch (line.lineType) {
      ...
      case 'ayah':
        return _buildJustifiedAyahLine(line); // Changed to justified
      default:
        return const SizedBox.shrink();
    }
  }

  // Build surah name header
Widget _buildSurahNameLine(MushafPageLine line) {
  // Gunakan surahNumber untuk mendapatkan glyph code
  final surahGlyphCode = line.surahNumber != null 
      ? _formatSurahIdForGlyph(line.surahNumber!) 
      : '';
  ...
  return Container(
    ...
    child: Stack(
      ...
      children: [
        ...
        Text(
          surahGlyphCode, // Menggunakan glyph code, bukan nameArabic
          style: const TextStyle(
            fontSize: 40,
            fontFamily: 'surah-name-v2',
            color: Colors.black,
  ...
  // Build basmallah line
  Widget _buildBasmallahLine(MushafPageLine line) {
    return Container(
      height: MushafTextRenderer.LINE_HEIGHT,
      alignment: Alignment.center,
      child: Text(
        '﷽',
        style: TextStyle(
  ...
  // NEW METHOD: Build justified ayah line with proper spacing
  // NEW METHOD: Build justified ayah line dengan deteksi angka Arab otomatis
  Widget _buildJustifiedAyahLine(MushafPageLine line) {
    if (line.ayahSegments == null || line.ayahSegments!.isEmpty) {
      return SizedBox(height: MushafTextRenderer.LINE_HEIGHT);
    }

    List<InlineSpan> spans = [];

    // Build word spans untuk entire line
    for (final segment in line.ayahSegments!) {
      ...
      final isCurrentAyat = ayatIndex >= 0 && ayatIndex == _currentAyatIndex;

      // Process setiap kata dalam segment
      for (int i = 0; i < segment.words.length; i++) {
        final word = segment.words[i];

        // Deteksi status API untuk kata ini
        APIWordResult? wordResult;
        if (isCurrentAyat && word.wordNumber - 1 < _currentAyatResults.length) {
          wordResult = _currentAyatResults[word.wordNumber - 1];
        }
        ...
        // IMPLEMENTASI BARU: Deteksi dan segment teks berdasarkan angka Arab
        final segments = segmentText(word.text);

        for (final textSegment in segments) {
          spans.add(
            TextSpan(
              text: textSegment.text + ' ', // Tambah spasi
              style: TextStyle(
                fontSize: _calculateOptimalFontSize(line),
                // LOGIKA FONT: Jika ada angka Arab, gunakan KFGQPCUthmanicScriptHAFSRegular
                fontFamily: textSegment.isArabicNumber
                ...
                backgroundColor:
                    _shouldShowWordBackground(status, isCurrentAyat)
                    ? _getAPIStatusColor(status, isCurrentAyat).withOpacity(0.2)
                    : null,
                fontWeight: textSegment.isArabicNumber
                    ? FontWeight
                          .bold // Angka Arab lebih tebal
                    : FontWeight.w500,
              ),
            ),
          );
    ...
    // Return justified text widget
    return MushafTextRenderer.renderJustifiedLine(
      wordSpans: spans,
      isCentered: line.isCentered,
      availableWidth:
          MediaQuery.of(context).size.width -
          (2 * MushafTextRenderer.PAGE_PADDING),
      context: context,
    );
  }

  // Calculate optimal font size based on word count
  double _calculateOptimalFontSize(MushafPageLine line) {
    if (line.ayahSegments == null) return 15.0;

    return 18.0;
  }

  static bool containsArabicNumbers(String text) {
    final arabicNumberPattern = RegExp(r'[٠-٩]+');
    return arabicNumberPattern.hasMatch(text);
  }

  /// Cek apakah teks adalah murni angka Arab ayat (tanpa huruf lain)
  static bool isPureArabicNumber(String text) {
    final trimmedText = text.trim();
    // Pattern untuk angka Arab dengan kemungkinan karakter ornamen Quran
    final pureNumberPattern = RegExp(r'^[٠-٩۝﴿﴾\s]+$');
    return pureNumberPattern.hasMatch(trimmedText) &&
        containsArabicNumbers(trimmedText);
  }

  /// Pisahkan teks menjadi bagian angka Arab dan teks biasa
  ...
  if (_currentPageAyats.isNotEmpty) {
    ...
    if (newIndex >= 0) {
      setState(() {
        ...
        _currentAyatNumber = _ayatList[newIndex].ayah;
      });
    }
  }
...
// TAMBAHKAN method ini setelah method _navigateToPage
void _preloadAdjacentPages() {
  // Pre-load next and previous pages in background
  if (_currentPage > 1) {
    _sqliteService.getMushafPageLines(_currentPage - 1);
  }
  if (_currentPage < 604) {
    _sqliteService.getMushafPageLines(_currentPage + 1);
  }
  ...
  // HELPER METHOD BARU: Build teks tersegmentasi untuk mode vertikal
  Widget _buildSegmentedText(
    ...
    final segments = segmentText(word);

    // Jika hanya satu segment dan bukan angka Arab, return teks biasa
    if (segments.length == 1 && !segments.first.isArabicNumber) {
      return Text(
        ...
        style: TextStyle(
          ...
          fontWeight: _getWordFontWeight(similarityScore, isCurrentWord),
          height: 1.5,
        ),
        textDirection: TextDirection.rtl,
      );
    ...
    // Jika ada segmentasi, gunakan RichText
    List<TextSpan> spans = [];
    ...
    for (final segment in segments) {
      spans.add(
        TextSpan(
          ...
          style: TextStyle(
            fontSize: 26,
            // LOGIKA FONT: Deteksi angka Arab otomatis dari database
            fontFamily: segment.isArabicNumber
                ? 'KFGQPCUthmanicScriptHAFSRegular'
                : 'Me_Quran',
            color: _getAPIWordTextColor(status, isCurrentWord),
            fontWeight: segment.isArabicNumber
                ? FontWeight
                      .bold // Angka Arab lebih menonjol
                : _getWordFontWeight(similarityScore, isCurrentWord),
            height: 1.5,
          ),
        ),
      );

      // Tambah spasi jika bukan segment terakhir
...
// TAMBAHKAN di bagian atas class QuranSQLiteService setelah line database declarations
...
class QuranSQLiteService {
  ...
  Future<List<AyatData>> getCurrentPageAyats(int pageNumber) async {
    ...
    // Collect all unique ayat on this page
    Map<String, AyatData> uniqueAyats = {};

    for (final layout in pageLayout) {
      // Only process ayah lines
      if (layout.lineType != 'ayah') continue;
      if (layout.firstWordId == null || layout.lastWordId == null) continue;

      // Get words for this line segment
      ...
      for (final word in lineWords) {
        ...
        if (!uniqueAyats.containsKey(key)) {
          // Create new ayat entry
          uniqueAyats[key] = AyatData(
            surah_id: word.surah,
            ayah: word.ayah,
            words: [],
            page: pageNumber,
            juz: _calculateJuz(word.surah, word.ayah),
            fullArabicText: '',
          );
        }
      }
    ...
    // Now get complete words for each unique ayat
    for (final key in uniqueAyats.keys) {
      final parts = key.split(':');
      final surahId = int.parse(parts[0]);
      final ayahNum = int.parse(parts[1]);

      // Get ALL words for this ayah (not just what's on this page)
      final completeWords = await getAyahWords(surahId, ayahNum);
  ...
  // NEW METHOD: Get properly structured page lines for mushaf rendering
Future<List<MushafPageLine>> getMushafPageLines(int pageNumber) async {
  ...
  // Keep cache size manageable (max 10 pages)
  ...
  // Helper method to get total words in an ayah
  Future<int> _getTotalWordsInAyah(int surahId, int ayahNumber) async {
    final words = await getAyahWords(surahId, ayahNumber);
    return words.length;
  }
...
// NEW DATA CLASSES FOR PROPER MUSHAF RENDERING
